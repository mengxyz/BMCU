...
extern void hex_out(uint8_t *data, int length);
...
void rsa_datas_init()
{
    // copy keys message about public key and private key to the flash RAM
    pk.bits = 2048;
    memcpy(&pk.modulus[RSA_MAX_MODULUS_LEN - sizeof(rsa_key_m)], rsa_key_m, sizeof(rsa_key_m));
    memcpy(&pk.exponent[RSA_MAX_MODULUS_LEN - sizeof(rsa_key_e)], rsa_key_e, sizeof(rsa_key_e));
    sk.bits = 2048;
    memcpy(&sk.modulus[RSA_MAX_MODULUS_LEN - sizeof(rsa_key_m)], rsa_key_m, sizeof(rsa_key_m));
    memcpy(&sk.public_exponet[RSA_MAX_MODULUS_LEN - sizeof(rsa_key_e)], rsa_key_e, sizeof(rsa_key_e));
    memcpy(&sk.exponent[RSA_MAX_MODULUS_LEN - sizeof(rsa_key_pe)], rsa_key_pe, sizeof(rsa_key_pe));
    memcpy(&sk.prime1[RSA_MAX_PRIME_LEN - sizeof(rsa_key_p1)], rsa_key_p1, sizeof(rsa_key_p1));
    memcpy(&sk.prime2[RSA_MAX_PRIME_LEN - sizeof(rsa_key_p2)], rsa_key_p2, sizeof(rsa_key_p2));
    memcpy(&sk.prime_exponent1[RSA_MAX_PRIME_LEN - sizeof(rsa_key_e1)], rsa_key_e1, sizeof(rsa_key_e1));
    memcpy(&sk.prime_exponent2[RSA_MAX_PRIME_LEN - sizeof(rsa_key_e2)], rsa_key_e2, sizeof(rsa_key_e2));
    memcpy(&sk.coefficient[RSA_MAX_PRIME_LEN - sizeof(rsa_key_c)], rsa_key_c, sizeof(rsa_key_c));
}
